...
def on_tree_select(event):
    ...
    elif item_text == "Episodic":
        open_btn = tk.Button(right_frame, text="Open", command=open_episodic_env)
        open_btn.pack(pady=10)
...
def open_episodic_env():
    epi_win = tk.Toplevel(root)
    epi_win.title("Episodic Environment Simulation")
    epi_win.geometry("700x350")

    main_frame = tk.Frame(epi_win)
    ...
    use_timestamps = tk.BooleanVar()

    tk.Label(input_frame, text="Total number of states to select from:").pack(anchor="w", pady=2)
    entry_total_states = tk.Entry(input_frame)
    entry_total_states.pack(fill=tk.X, pady=5)

    tk.Label(input_frame, text="Number of states to select:").pack(anchor="w", pady=2)
    entry_num_select = tk.Entry(input_frame)
    entry_num_select.pack(fill=tk.X, pady=5)

    tk.Checkbutton(input_frame, text="Include Timestamps", variable=use_timestamps).pack(anchor="w", pady=5)

    def simulate_episodes():
        try:
            n = int(entry_total_states.get())
            a = int(entry_num_select.get())
            if a > n:
                messagebox.showerror("Error", "Cannot select more states than available.")
                return
            selected = random.sample(range(n), a)
            result_text.config(state="normal")
            result_text.delete("1.0", tk.END)
            if use_timestamps.get():
                result_text.insert(tk.END, "Selected States with Timestamps:\n\n")
                now = datetime.datetime.now()
                for i, val in enumerate(selected):
                    timestamp = now + datetime.timedelta(seconds=i * random.randint(1, 5))
                    result_text.insert(tk.END, f"State: {val} at {timestamp.strftime('%Y-%m-%d %H:%M:%S')}\n")
            else:
                result_text.insert(tk.END, f"Selected States:\n{selected}")
            result_text.config(state="disabled")
        except ValueError:
            messagebox.showerror("Input Error", "Please enter valid integers.")

    tk.Button(input_frame, text="Simulate", command=simulate_episodes).pack(pady=10)

    result_text = tk.Text(output_frame, state="disabled", wrap="word")
...
def open_sequential_env():
    ...
    def function1(number, operand): return number + operand
    def function2(number, operand): return number * operand
    def function3(number, operand): return number - operand
    def function4(number, operand): 
        if operand == 0: raise ValueError("Cannot divide by zero.")
        return number / operand
    def function5(number, operand): return number ** operand
    def function6(number, operand): 
        if number <= 0: raise ValueError("Logarithm undefined for non-positive numbers.")
        return math.log10(number)
    def function7(number, operand): 
        if number < 0: raise ValueError("Square root undefined for negative numbers.")
        return math.sqrt(number)
    def function8(number, operand): return round(number)

    operation_functions = {
        1: function1, 2: function2, 3: function3, 4: function4,
        5: function5, 6: function6, 7: function7, 8: function8
    }
    ...
    def draw_step_circle(step_num, value):
        ...
        canvas.create_oval(x_center - radius_x, y - radius_y, x_center + radius_x, y + radius_y,
                           fill="#c0392b", outline="black", width=2)
        canvas.create_oval(x_center - radius_x + 5, y - radius_y + 5,
                           x_center + radius_x - 5, y + radius_y - 5,
                           fill="#e74c3c", outline="")
        display_value = round(value, 6) if isinstance(value, float) else value
        canvas.create_text(x_center, y, text=f"{display_value}", fill="white")
        ...
        if step_num > 0:
            ...
            if row == prev_row:
                canvas.create_line(prev_x + radius_x, prev_y, x_center - radius_x, y,
                                   arrow=tk.LAST, width=4, fill="black", arrowshape=(20, 25, 10), smooth=True)
            else:
                offset_x, offset_y = 25, 15
                canvas.create_line(prev_x + radius_x, prev_y, prev_x + radius_x + offset_x, prev_y,
                                   width=4, fill="black", smooth=True)
                canvas.create_line(prev_x + radius_x + offset_x, prev_y,
                                   prev_x + radius_x + offset_x, y - radius_y - offset_y,
                                   width=4, fill="black", smooth=True)
                canvas.create_line(prev_x + radius_x + offset_x, y - radius_y - offset_y,
                                   x_center - radius_x, y - radius_y - offset_y,
                                   arrow=tk.LAST, width=4, fill="black", arrowshape=(20, 25, 10), smooth=True)
